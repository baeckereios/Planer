...
## Erweiterung: Drei-Schichten-Architektur

> Entschieden: April 2026 · Session Schichtplaner-Umbau

### Grundprinzip

**Eine Config-Datei, drei Schichten.** Keine separaten JSON-Dateien pro Schicht. Der Algorithmus filtert anhand von `currentSchicht`.

### Personen-Feld `stammschicht`

```json
{ "id": "lars",  "name": "Lars",  "stammschicht": "nacht", ... }
{ "id": "anna",  "name": "Anna",  "stammschicht": "frueh", ... }
{ "id": "petra", "name": "Petra", "stammschicht": "kond",  ... }
{ "id": "felix", "name": "Felix", "stammschicht": "azubi", ... }
```

Gültige Werte: `"nacht"` · `"frueh"` · `"kond"` · `"azubi"`

### Positionen-Feld `schicht`

```json
{ "id": "ofen",       "schicht": "nacht", ... }
{ "id": "verpackung", "schicht": "frueh", ... }
{ "id": "torte",      "schicht": "kond",  ... }
```

Gültige Werte: `"nacht"` · `"frueh"` · `"kond"` · `"beide"`

### Namespace in freieTage / schichtplaene.js

`currentSchicht` wird als Präfix in den WeekKey eingebaut:

```
"nacht_2026-W16": { "lars": { tag: "samstag", typ: "wunsch" } }
"frueh_2026-W16": { "anna": { tag: "mittwoch", typ: "auto" } }
"kond_2026-W16":  { "petra": { tag: "freitag", typ: "auto" } }
```

Keine Kollisionen zwischen Schichten. Eine einzige `schichtplaene.js` für alle drei.
...
- Frühschicht
- Konditorei
- *(Azubi erscheint nicht als eigene Planungseinheit — Azubis werden dem Plan der jeweiligen Schicht zugeordnet)*

### Config-Editor — Personen-Akkordeons

Vier Gruppen in `schichtplaner_config.html`:

```
▼ Nachtschicht  (n Personen)
▼ Frühschicht   (n Personen)
▼ Konditorei    (n Personen)
▼ Azubi         (n Personen)
```

Azubi-Gruppe: vereinfachte Ansicht, kein Guthaben, kein Stammkraft-Feld.

### Offene Aufgaben (aus dieser Entscheidung)

1. `stammschicht`-Feld zu allen bestehenden Personen in `schichtplaner_config.json` ergänzen
2. `schicht`-Feld zu allen bestehenden Positionen ergänzen
3. `loadConfig()` filtert Personen + Positionen nach `currentSchicht`
4. WeekKey-Namespace: `getWeekKey()` bekommt `currentSchicht` als Parameter
5. `schichtplaene.js` und `urlaub_krank.js` sind schicht-namespace-aware
6. Config-Editor: Personen-Tab auf vier Akkordeons umbauen
7. Schicht-Dropdown: Option `frueh` und `kond` aktivieren (bisher nur Stub)

---

*Entschieden: April 2026 · [Claude] + [Ulf]*

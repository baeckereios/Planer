...
function _checkAndLockFroster(shouldLock) {
    ...
    // DZ_latestInvTs kann 0 sein, obwohl BOS_INVENTUR bereits geladen ist —
    // dann war inventurdaten.js beim DZ_init-Retry noch nicht fertig.
    if (window.BOS_INVENTUR && window.BOS_INVENTUR.products) {
        ...
        if (maxTs > 0 && (Date.now() - maxTs) <= 24 * 3600000) {
            // Inventur ist da und frisch — DZ_latestInvTs nachträglich korrigieren
            DZ_latestInvTs = maxTs;
            var d2  = new Date(maxTs);
            var pad2 = function(n) { return n < 10 ? '0' + n : n; };
            var ageH2 = Math.floor((Date.now() - maxTs) / 3600000);
            var dot2 = document.getElementById('inv-dot');
            var txt2 = document.getElementById('inv-text');
            var bar2 = document.getElementById('inv-bar');
            txt2.textContent = 'Inventur: ' + pad2(d2.getDate()) + '.' + pad2(d2.getMonth() + 1) + '. ' +
                               pad2(d2.getHours()) + ':' + pad2(d2.getMinutes()) + ' — ' + ageH2 + ' Std. alt';
            dot2.className = 'inv-dot'; // grün
            bar2.classList.remove('warn');
            return; // NICHT sperren
        }
    }
